...
Tooling
- Package manager/runtime: use `bun`. Do not introduce npm/pnpm/yarn lockfiles; keep `bun.lock` as the single source of truth.
...
- shadcn/ui: add components using `bunx shadcn@latest`. Place generated files under `src/components/` following shadcn's structure.
